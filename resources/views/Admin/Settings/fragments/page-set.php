                ...
                <div class="col-sm-5 offset-1">
                        <div class="input-group">
                                <select name="Page/search" id="search_page" class="form-control">
                                        ...
                                        <option value="<?php echo $page->ID; ?>" <?php if (Brainsugar()->options->get('Pages.search') == $page->ID) {
    echo ' selected="selected"';
} ?>><?php echo get_the_title($page); ?></option>
                                        <?php } ?>
                                </select>
                        </div>
                ...
                <div class="col-sm-5 offset-1">
                        <div class="input-group">
                                <select name="Page/check_out" id="check_out" class="form-control">
                                        <option><?php esc_html_e('Select page', 'bshb'); ?></option>
                                        <?php foreach ($pages as $page) { ?>
                                        <option value="<?php echo $page->ID; ?>" <?php if (Brainsugar()->options->get('Pages.check_out') == $page->ID) {
    echo ' selected="selected"';
} ?>><?php echo get_the_title($page); ?></option>
                                        <?php } ?>
                                </select>
         ...
         <div class="row my-5">
                <div class="col-sm-4">
                        <h5 class="font-weight-light"><?php esc_html_e('Reservation Confirmation', 'bshb'); ?></h5>
                        <p class="setting-desc mb-4"><?php esc_html_e('Payment status and summary of reservation will be displayed on this page.', 'bshb'); ?></p>
                </div>
                <!-- checkout page -->
                <div class="col-sm-5 offset-1">
                        <div class="input-group">
                                <select name="Page/reservation_confirmation" id="reservation_confirmation" class="form-control">
                                        <option><?php esc_html_e('Select page', 'bshb'); ?></option>
                                        <?php foreach ($pages as $page) { ?>
                                        <option value="<?php echo $page->ID; ?>" <?php if (Brainsugar()->options->get('Pages.reservation_confirmation') == $page->ID) {
    echo ' selected="selected"';
} ?>><?php echo get_the_title($page); ?></option>
                                        <?php } ?>
                                </select>
